...
class VisionTransformer_HiCroPL(nn.Module):
    def __init__(
        ...
        # how many layers get prompts injected
        self.prompt_till_layer_visual = design_details["vision_depth"]
    ...
    def forward(
        ...
        """
        Default (backward compatible): returns global CLS embedding, shape [B, output_dim].

        If return_patch_tokens=True:
            returns (global_feat, patch_tokens)
            - global_feat: [B, output_dim]
            - patch_tokens: [B, N, width] by default (project_patch_tokens=False)
                           [B, N, output_dim] if project_patch_tokens=True

        If return_all_tokens=True:
            returns dict with keys:
              - "global": [B, output_dim]
              - "patch_tokens": [B, N, width] or [B, N, output_dim]
              - "all_tokens": [B, L, width]  (LN applied; *not* projected)
        """
        ...
        cls = cls.unsqueeze(0).unsqueeze(0).expand(x.shape[0], 1, -1)  # [B, 1, width]
        x = torch.cat([cls, x], dim=1)          # [B, 1+N, width]

        # Positional embedding (expects fixed resolution)
        x = x + self.positional_embedding.to(x.dtype)

        # Append shallow visual prompts (trainable) if enabled
        if self.VPT_shallow:
            # img_prompts: [1, n_ctx, width] typically
            visual_ctx = img_prompts.expand(x.shape[0], -1, -1).half()
            x = torch.cat([x, visual_ctx], dim=1)  # [B, 1+N+n_ctx, width]
        else:
            assert self.prompt_till_layer_visual == 0

        # Transformer expects [L, B, D] and returns [x, cross_prompts_visual_deeper]
        x = self.ln_pre(x)
        ...
        # Apply LN on *all* tokens (token 0 stays identical to old behavior)
        x_tokens = self.ln_post(x)  # [B, L, width]

        # Global CLS embedding (projected like original)
        global_feat = x_tokens[:, 0, :]  # [B, width]
        if self.proj is not None:
            global_feat = global_feat @ self.proj  # [B, output_dim]

        # Backward-compatible return
        if not return_patch_tokens and not return_all_tokens:
            return global_feat

        # Patch token count is fixed by positional embedding length (no prompts included there)
        n_patches = int(self.positional_embedding.shape[0] - 1)  # grid^2
        patch_tokens = x_tokens[:, 1:1 + n_patches, :]           # [B, N, width]

        if project_patch_tokens and self.proj is not None:
            patch_tokens_out = patch_tokens @ self.proj          # [B, N, output_dim]
        else:
            patch_tokens_out = patch_tokens

        if return_all_tokens:
            return {
                "global": global_feat,
                "patch_tokens": patch_tokens_out,
                "all_tokens": x_tokens,  # LN-applied tokens in width space
            }

        return global_feat, patch_tokens_out
